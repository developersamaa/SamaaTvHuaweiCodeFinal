...
public class WorldNews extends Fragment implements SwipeRefreshLayout.OnRefreshListener
{
    ...
    String globNewsCat;
    private ListingsAdapter world_adapter;
    ...
    private void fetchWorldIfNotif()
    {
        ...
        if (InternetConnection.checkConnection(getActivity()))
        {
            ...
            Call<NewsCategoryList> call = api.getGlobalNews();
            ...
            call.enqueue(new Callback<NewsCategoryList>()
            {
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (world_list != null)
                        {
                            world_adapter = new ListingsAdapter(getActivity(), world_list, "Global");
    ...
    {
        world_list = DataCache.retrieveGlobList(getActivity());
        if (world_list != null)
        {
            world_adapter = new ListingsAdapter(getActivity(), world_list, "Global");
            world_recycle.setAdapter(world_adapter);
    ...
    public void onResume()
    {
        ...
        new Handler().postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                SamaaAppAnalytics.getInstance().trackScreenView("Global News Screen");
                SamaaFirebaseAnalytics.syncSamaaAnalytics(getActivity(), "Global News Screen");
            }
        }, 6000);
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (world_list != null)
                        {
                            world_adapter = new ListingsAdapter(getActivity(), world_list, "Global");
                            world_recycle.setAdapter(world_adapter);
                            DataCache.saveGlobList(getActivity(), world_list);
