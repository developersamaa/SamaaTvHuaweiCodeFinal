...
public class PakistanNews extends Fragment implements SwipeRefreshLayout.OnRefreshListener
{
            ...
            call.enqueue(new Callback<NewsCategoryList>()
            {
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (pak_list != null)
                        {
                            pak_adapter = new ListingsAdapter(getActivity(), pak_list, "National");
    ...
    private void fetchPaklIstPreference()
    {
        ...
        pak_list = DataCache.retrievePakList(getActivity());
        if (pak_list != null)
        {
            pak_adapter = new ListingsAdapter(getActivity(), pak_list, "National");
            pak_recycle.setAdapter(pak_adapter);
    ...
    {
        rootView = (ViewGroup) inflater.inflate(R.layout.fragment_recyclerview, container, false);
        return rootView;
    }
    ...
    public void onResume()
    {
        ...
        new Handler().postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                SamaaAppAnalytics.getInstance().trackScreenView("Pakistan News English");
                SamaaFirebaseAnalytics.syncSamaaAnalytics(getActivity(), "Pakistan News English");
            }
        }, 6000);
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (pak_list != null)
                        {
                            pak_adapter = new ListingsAdapter(getActivity(), pak_list, "National");
                            pak_recycle.setAdapter(pak_adapter);
                            DataCache.savePakList(getActivity(), pak_list);
