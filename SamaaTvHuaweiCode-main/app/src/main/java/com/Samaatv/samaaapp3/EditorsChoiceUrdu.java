...
import com.Samaatv.samaaapp3.adapter.EditorsChoiceAdapterListingUrdu;
...
public class EditorsChoiceUrdu extends Fragment implements SwipeRefreshLayout.OnRefreshListener
{
    ...
    private EditorsChoiceAdapterListingUrdu editor_adapter;
    private ArrayList<Contact> editor_list;
    private View parentView;
    private String editNewsCat;

    private Boolean isStarted = false;
    private Boolean isVisible = false;

    public EditorsChoiceUrdu()
    {
    ...
    private void fetchEditListPreference()
    {
        ...
        editor_list = DataCacheUrdu.retrieveEditList(getActivity());

        if (editor_list != null)
        {
            editor_adapter = new EditorsChoiceAdapterListingUrdu(getActivity(), editor_list);
    ...
    public void onResume()
    {
        ...
        new Handler().postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                SamaaAppAnalytics.getInstance().trackScreenView("Editor's Choice News Urdu");
                SamaaFirebaseAnalytics.syncSamaaAnalytics(getActivity(), "Editor's Choice News Urdu");
            }
        }, 6000);
    ...
    private void fetchEditList()
    {
        if (InternetConnection.checkConnection(getActivity()))
        {
            ...
            Call<NewsCategoryList> call = api.getEditorsNewsUrdu();
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (editor_list != null)
                        {
                            editor_adapter = new EditorsChoiceAdapterListingUrdu(getActivity(), editor_list);
                            editor_recycle.setAdapter(editor_adapter);
                            DataCacheUrdu.saveEditList(getActivity(), editor_list);
