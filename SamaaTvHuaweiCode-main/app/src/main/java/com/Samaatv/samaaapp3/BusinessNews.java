...
public class BusinessNews extends Fragment implements SwipeRefreshLayout.OnRefreshListener
{
    ...
    String econNewsCat;
    private ListingsAdapter business_adapter;
    ...
    private void fetchEconListPreference()
    {
        ...
        business_list = DataCache.retrieveEconomyList(getActivity());

        if (business_list != null)
        {
            business_adapter = new ListingsAdapter(getActivity(), business_list, "Economy");
    ...
    public void onResume()
    {
        ...
        new Handler().postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                SamaaAppAnalytics.getInstance().trackScreenView("Business News English");
                SamaaFirebaseAnalytics.syncSamaaAnalytics(getActivity(), "Business News English");
            }
        }, 6000);
    }
    ...
    private void fetchEconList()
    {
        if (InternetConnection.checkConnection(getActivity()))
        {
            ...
            Call<NewsCategoryList> call = api.getEconomyNews();
                ...
                public void onResponse(Call<NewsCategoryList> call, Response<NewsCategoryList> response)
                {
                    ...
                    if (response.isSuccessful())
                    {
                        ...
                        if (business_list != null)
                        {
                            ...
                            business_adapter = new ListingsAdapter(getActivity(), business_list, "Economy");
                            business_recycle.setAdapter(business_adapter);
                            DataCache.saveEconomyList(getActivity(), business_list);
                        }
                        else
                        {
                            business_list = new ArrayList<Contact>();
